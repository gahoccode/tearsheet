...
class ChartDataService:
    ...
    def create_portfolio_performance_data(
        ...
        try:
            ...
            data = [{
                'x': [date.strftime('%Y-%m-%d') for date in cumulative_returns.index],
            ...
            layout = {
                'title': {'text': title, 'x': 0.5, 'font': {'size': 16, 'color': '#1a237e'}},
                'xaxis': {'title': 'Date', 'showgrid': True, 'gridcolor': '#f0f0f0'},
                'yaxis': {'title': 'Cumulative Return (%)', 'showgrid': True, 'gridcolor': '#f0f0f0'},
                'hovermode': 'x unified',
                'showlegend': True,
                'height': 400,
                'paper_bgcolor': 'white',
    ...
    def create_drawdown_data(
        ...
        try:
            ...
            layout = {
                'title': {'text': title, 'x': 0.5, 'font': {'size': 16, 'color': '#1a237e'}},
                'xaxis': {'title': 'Date', 'showgrid': True, 'gridcolor': '#f0f0f0'},
                'yaxis': {'title': 'Drawdown (%)', 'showgrid': True, 'gridcolor': '#f0f0f0'},
                'hovermode': 'x unified',
                'showlegend': True,
                'height': 400,
                'paper_bgcolor': 'white',
    ...
    def create_metrics_dashboard_data(
        ...
        try:
            ...
            metric_colors.append('#2ca02c' if sharpe > 1 else '#ff7f0e' if sharpe > 0 else '#d62728')
    ...
    def create_ratio_comparison_data(
        ...
        try:
            ...
            for symbol, df in ratio_data.items():
                if metric in df.columns and not df[metric].isna().all():
                    data.append({
                        ...
                        'type': 'bar',
                        'name': symbol,
                        'marker': {'color': self.default_colors[color_idx % len(self.default_colors)]},
                        'hovertemplate': f'<b>{symbol}</b><br>{metric}: %{{y:.2f}}<br>Period: %{{x}}<extra></extra>'
                    })
                    color_idx += 1

            layout = {
                'title': {'text': f"{title} - {metric}", 'x': 0.5, 'font': {'size': 16, 'color': '#1a237e'}},
                'xaxis': {'title': 'Period', 'showgrid': True, 'gridcolor': '#f0f0f0'},
                'yaxis': {'title': f'{metric} Ratio', 'showgrid': True, 'gridcolor': '#f0f0f0'},
                'barmode': 'group',
                'hovermode': 'x unified',
                'height': 400,
                'showlegend': True,
            ...
            raise AnalysisError(f"Failed to create ratio comparison chart data: {str(e)}")